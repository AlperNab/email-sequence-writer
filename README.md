...
**Domain:** `Marketing / Lifecycle Email` · **Suite:** `General Automation Suite` · **Accent:** `#93c5fd`
...
**Email Sequence Writer** is a standalone, browser-based AI workflow app for **Marketing / Lifecycle Email**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `offer`<br>Offer | text | Yes | — | Affects input: Offer. |
| `persona`<br>persona | text | Yes | — | Affects input: persona. |
...
| `goal`<br>goal | text | No | — | Affects customization: goal. |
| `funnel_stage`<br>funnel stage | text | No | — | Affects customization: funnel stage. |
| `sequence_length`<br>sequence length | text | No | — | Affects customization: sequence length. |
| `tone`<br>tone | text | No | — | Affects customization: tone. |
| `brand_voice`<br>brand voice | text | No | — | Affects customization: brand voice. |
| `esp_format`<br>ESP format | select | No | — | Affects customization: ESP format. |
| `personalization_fields`<br>personalization fields | text | No | — | Affects customization: personalization fields. |
| `send_cadence`<br>send cadence | text | No | — | Affects customization: send cadence. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9119
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9119`. |
...
```bash
curl http://127.0.0.1:9119/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9119/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9119 uvicorn app.main:app --host 0.0.0.0 --port 9119
```
...
- `plugins/email-sequence-writer.json` for plugin metadata.
